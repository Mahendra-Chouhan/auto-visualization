...
def init_session():
    ...
    if "Plaid_df" in st.session_state:
        del st.session_state["Plaid_df"]
    st.session_state["Plaid_visualizations"] = []
    st.session_state["Plaid_KPIs"] = utils.get_kpis("Plaid")
...
def preload_visualization(application, lida, textgen_config, except_ids=[]):
        ...
        for visuals in st.session_state[f"{application}_visualizations"]:
            if visuals['id'] not in except_ids:
                with st.container(border=True):
                    ...
                    st.text_input("Prompt:", value=visuals["kpi_query"], key=visuals["id"], on_change=visual_edit,
                                  args=(lida, textgen_config, visuals["id"], st.session_state[f"{application}_summary"], application))
...
def Preload(lida, textgen_config, application, except_ids=[]):
    ...
    if application == "CIDDS" and f"{application}_df" in st.session_state:
        with st.container(border=True):
            st.dataframe(st.session_state[f"{application}_df"].head(7), hide_index=True, use_container_width=True)
        preload_visualization(application, lida, textgen_config, except_ids)
        
    if application == "Plaid" and f"{application}_df" in st.session_state:
